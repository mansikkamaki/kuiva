...
$dE = 2\,\mathrm{Re} \sum_{p>q} G_{pq} \kappa_{pq}$ with the anti-Hermitian gradient
...
Convergence therefore requires **both** $|g| < \texttt{conv\_grad}$ and a converged energy,
...
- **convergence means $|g| < \texttt{conv\_grad}$ *and* a proposal that refuses** — a
