...
**Rows are spin-blocked.** A spinor coefficient array has shape $(2 n_{\mathrm{bas}},
n_{\mathrm{spinor}})$ over one underlying real scalar basis (the AO basis, or the
...
\qquad k_1 < k_2 < \cdots < k_N,
$$

so that both $a_p|I\rangle$ and $a^\dagger_p|I\rangle$ carry the sign
$(-1)^{\#\{\text{occupied } k < p\}}$ — the standard Slater–Condon phase convention
...
[[179]](references.md#r179), and, for electric dipoles, $\mathrm{Tr}_{\mathrm{block}}(d_i
d_j)$ and block-to-block line strengths $\sum |d_{IJ}|^2$. Element-by-element comparison of
