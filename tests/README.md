...
| `tests/generate/*.py` | One generation script per committed reference (`tier1_pyscf`, `tier2_molcas`, `tier2_dirac`, `tier2_kuiva`, `x2camf_*`, `amf_*`, `nevpt2_*`, `tier3_invariants`, `crosscheck_external_basis`) plus benchmark/characterization scripts that write `temp/` rather than a reference (`bench_*`, `*_study`, `dlu_accuracy`, `cholesky_threshold`, `amf_open_shell_cost`, `spec_gaps_*`, `dmrg_memory_plan` — the last one samples peak RSS per phase of a tensor-network sweep against what the ledger declared, which is how the layer's largest array was found). Each script's docstring states what it produces, its cost, and its pinned comparison variables. |
...
| `tests/test_non_kramers.py` | **The integer-spin case, end to end** (slow): linear FeCl₂, whose non-Kramers ground doublet has an analytic `g_z` and transverse g values that are zero by symmetry rather than small. Every other committed system is odd-electron and therefore Kramers protected, so nothing else in the suite can test any of it — including the tunnelling gap, which appears only when the axis is broken. |
| `tests/test_localize.py`, `tests/test_broken_symmetry.py` | The site partition and the spin-polarized starting guess: stretched H₂ and a bare Ti(3+) pair, both seconds. What is asserted is what can silently be false — that a localization changes no CI energy, that the pairs survive it, that an ordinary UHF on a coupled dimer stays symmetric, and that a broken-symmetry solution carries the spin pattern it was *asked* for rather than merely some pattern. |
...
| `tests/test_ci_kramers.py` | The Kramers-restricted (time-reversal-adapted) CI, against the general complex path it is validated *against* rather than compared *to*. Tier 0 for the sign algebra (an explicit operator-string derivation sharing no line with the popcount arithmetic) and for `[H, T] = 0` **with a control that fails it**; Tier 1 for energies, RDMs and a phase-invariant reduction of the transition densities. The factor of two is pinned as an inequality on applications of `H`, **with its boundary** — it does not exist at two Kramers pairs, and a test that claimed otherwise would be asserting something false. |
| `tests/test_symmetry.py` | Abelian double-group symmetry, end to end. Tier 0 for the label algebra and the stored character tables (row orthogonality, the boson/fermion grading, the spin-1/2 shift); Tier 0 for the **printed** characters against `tr U(g)` computed from the run's own AO operator matrices, which is what stops the printed conventions and the arithmetic from drifting apart; Tier 1 against **PySCF's own** `orbsym` on a molecule PySCF can label, and against a **dense diagonalization of the sector** — a reference sharing nothing with the sector solver, so it can fail on the new code alone. Plus the refusals (a group the geometry does not have, orbitals that are not symmetry-pure, an empty sector) and, marked slow, the masked CASSCF and the labelled tensor network against the exact CI. |
| `tests/test_symmetry_nonabelian.py` | The **non-abelian classification** layer. Tier 0 for the group construction, against the theorems a finite group must satisfy — the double group is twice the point group, the characters are orthogonal both ways, `sum d^2` matches the order and half of it sits in the double-valued rows — which is what makes a *computed* character table as trustworthy as a printed one; the irrep **names** against the published tables, that being the one place a transcription belongs, in a test that can fail. Tier 0 for the general operator construction against the **landed abelian one** (two independent builds of the same `U(g)`, element for element) and for `U(g)` on a CI vector against `expm` of the one-body operator built from the determinant strings — two constructions with no line of code in common. Tier 1 for the multiplets of `N2+`, whose assignment is fixed by the spectroscopy of the molecule and by nothing in this program, plus the refusals: an active space the group takes states out of (which **degrades**, because a labelling may never break a run) and a state count that cuts a multiplet (which does not). |
| `tests/test_symmetry_kramers.py` | The two symmetries **together**, which is the one file that owns the combination. Every case turns on one fact: time reversal *conjugates* an irrep label, so the indivisible unit of a Kramers-restricted per-irrep selection is a conjugate **pair** of sectors. Checked against the general path's own spectrum and its RDMs on the same states, with the pair degeneracy asserted at exactly zero; plus the refusals the combination adds, and the demonstration that the combined mode makes well posed a request the general path's state-averaging gate refuses. |
| `tests/test_dmrg_checkpoint.py`, `tests/test_dmrg_extrapolate.py`, `tests/test_nevpt2_network.py` | The tensor-network production surface added in v0.29.0. The network-state checkpoint round-trips **bitwise** and its restart is a warm start whose losses degrade to a cold start with a warning; the bond-dimension series is reported exact when the truncation never bit and its extrapolate is never presented without the series; and the network-backed NEVPT2 provider is pinned primitive-by-primitive and class-by-class against the CI provider on a reference both represent exactly — with the partiality of its E2 (six of eight classes) asserted to be loud rather than absent. |
| `tests/test_projection.py` | Carrying a converged orbital set from one basis set onto another (`kuiva.orth.project`), which is a feature whose every failure mode is silent: a wrong projection still delivers an orthonormal set of the right shape that starts a calculation which converges, and the only symptom is a run that takes longer. So the assertions are properties rather than outcomes — projecting a basis onto **itself** is the identity (the one case whose answer is known without trusting the module), orthonormality is checked in the **target's own** metric, Kramers pairing is checked as a coefficient identity needing no metric, the retained norm is recomputed independently from the two overlaps, the principal overlaps are shown invariant under a random unitary inside the active space, and the completed columns are checked to be the target's own lowest orbitals in energy order — that last being exactly what one wrong-signed comparison destroyed while changing no energy. Plus the refusals: a target too small to hold the active space, a linearly dependent projected block, a mismatched molecule, and the class-API combinations that would define a different calculation. |
| `tests/test_scf_controls.py` | The molecular SCF's convergence controls, its orbital re-use and its non-convergence refusal. Four groups, each testing a mechanism rather than the observable it was found through: the **named-value checks**, because PySCF substitutes `minao` silently for an `init_guess` it does not know and a DIIS variant is a class assignment; the **starting guess**, asserted as `Tr(D S) = N` on both routes (a projector is not unitary, so the failure mode is a density with the wrong number of electrons in it, which a converged energy would not catch); **stability**, on a Ni atom whose ROHF converges cleanly onto a saddle point 0.30 Eh above the solution one rotation away, asserted as a *lower* energy rather than a flag; and the **refusal**, whose case (CrO from a one-electron guess) is measured not to converge at 100 cycles either, so it is a genuine failure and not a budget artefact. |
| `tests/test_spin_assign.py` | `<S^2>` over CI states and the term assignment built on it. The load-bearing case is the **out-of-CAS-space** part of `S|I>`: a random unitary is applied to the spin operator so the orbital spaces are genuinely not spin-separable, and the result is compared against an explicit determinant-basis `S_k` over *every* orbital — a brute-force construction sharing no code with the implementation, which the correct value matches to 2e-16 and the uncorrected one misses by 93 % of itself. That gap is asserted as an inequality, so dropping the correction fails the test rather than quietly shrinking a number. The atoms are the physical checks: O `2p^4` spin-free must give integral `S(S+1)` (a requirement, not a tolerance) and B `2p^1` must come back `^2P_1/2` / `^2P_3/2` with `L` recovered as an integer from a g value carrying its own known 1e-3 error. The assignment's **refusals outnumber its successes**, deliberately — a crystal-field doublet, a spin-mixed block, a Landé-consistent but triangle-condition-violating `(L, S, J)`, a `J = 0` block, and two analyses blocked differently — because a labeller that always produces a label is the failure mode that matters. |
| `tests/test_orbopt_kramers.py` | The Kramers constraint on the CASSCF rotation and the stability test that decides whether to keep it. The fast tests are all **mechanism**, because no cheap synthetic reproduces the drift or the instability (60 configurations tried): the packed projector against the matrix form, its odd complement, `exp` of an even generator preserving pairing while a general one destroys it, the time-odd Davidson against a dense diagonalization of the same restricted operator, the deterministic eigenvector sign, and one deterministic *model* of the drift. The one real instability — N₂ CAS(6,8), where the constraint stops 0.64 Eh above the answer — is a `slow` test, and it compares **converged** energies rather than trajectories because the unconstrained route there is a ridge. |
| `tests/test_avas.py` | The AVAS active-space construction. Three invariances, each chosen because a plausible wrong implementation breaks exactly one: the SCF **density does not move** (which fails if the rotation is not confined to groups of equal occupation), **Kramers pairing survives** (which fails if the barred partners take `V` instead of `conj(V)` — asserted by showing the wrong form *does* break it while staying orthonormal, so the right form's success means something), and the projection eigenvalues **sum to the reference dimension**, which no threshold or ordering choice can satisfy by accident. Plus contiguity of the selected block, which is what the per-occupation ordering exists for, and the refusals: a threshold nothing clears, `max_pairs`, a shell the reference basis cannot supply, a missing atomic reference, and the stage-level combinations that would state the active space twice. |
| `tests/test_autocas_targets.py`, `tests/test_autocas_multiplets.py`, `tests/test_autocas_centres.py`, `tests/test_autocas_candidates.py`, `tests/test_autocas_protocol.py` | **Automatic active-space selection.** The vocabulary round-trips (every spelling parses to its dataclass and writes itself back) and refuses what it must; the Hund floors are checked against tabulated free-ion ground terms and against the suite's own committed degeneracy patterns. Centre detection is tested through the two systems where one condition alone gives a well-formed calculation about nothing — `zn2p`, whose **filled** 3d is the most convincing centre in any population table, and `cecl3`, whose frontier orbitals clear the cut on the 5d as convincingly as on the 4f — plus the pooled case, which is the normal one for equivalent metals. The candidate constructors are compared against the committed `character=` spaces two ways, **which pairs** and **what they span**, because AVAS rotates and only the first is exact. The protocol's decisions run against a **scripted probe**, since what is under test is the conditional and not the pre-optimization: keep, drop, inconclusive, a failed probe, an over-budget class, a core that refuses. Three load-bearing negatives: the tie rule must not fire when there is no tie (reading a *selection order* as a spectrum once rounded every bounded count out to its whole pool), a budget must be re-evaluated at the filling it is asked about, and on `ti2cl6` the spectrum rule and an absolute entanglement cut must reach **different** answers about the bridge — a keep/drop rule whose two candidate implementations never disagree is not a rule anybody chose. |
| `tests/test_state_window.py`, `tests/test_dmrg_window.py` | Selecting the states by an **energy cutoff** rather than a count. The rule is tested where it can be tested exactly — synthetic ascending spectra covering every branch (a cutoff on a gap, inside a cluster, chaining to the cap, spanning the space, Kramers pairs straddling the cutoff, the per-irrep merge, the dead band, the unit conversions) — and the ladder against a *dense* spectrum: on a space small enough to diagonalize, the count the solver resolves to must equal the count the same rule reads off `eigvalsh`, which is what gives a resolved count a target at all. The network file adds the three things that route does differently: the converged-network witness, the cold growth, and the two-site capacity that makes a window **refusable** on a topology too finely partitioned to hold a witness. The load-bearing negative is the generic-vector one — a ladder asked for its witness *without* generic starting vectors misses a sector it must find, so the guard is shown to be able to fail. |
